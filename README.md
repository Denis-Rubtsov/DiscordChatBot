...
- на упоминания (`@Мурка ...`) и ответы (reply) на её сообщения в текстовых каналах сервера — а после первого упоминания в канале начинает отвечать на все сообщения в нём, без повторных упоминаний.

Контекст диалога хранится отдельно по каждому каналу (последние 50 обменов репликами) и вместе со списком «активных» каналов сохраняется на диск (`StateFile`, по умолчанию `botstate.json`) — переживает перезапуски.

**Остальные умения:**
- `!мнение @кто-то` (или `!мнение имя`) — Мурка публично рассказывает, что думает об участнике, по своей памяти отношений.
- Понимает присланные картинки (вложения-изображения уходят в модель вместе с текстом) и рисует свои по просьбе «нарисуй ...» (`OpenAI__ImageModel`, по умолчанию `gpt-image-1`).
- Изредка (≈8% сообщений, на которые она не отвечает) ставит эмодзи-реакцию по смыслу сообщения.
- Может раз в 3-6 часов сама вставить короткую реплику в живой разговор, идущий без неё.
- Приветствует новичков на сервере (нужен привилегированный **Server Members Intent**, см. настройку; аварийное отключение — `Discord__GuildMembersIntent=false`).
- Следит за правилами сервера из системного промпта: на явный мат/политику может мягко напомнить пункт правил (быстрый словарный префильтр + подтверждение моделью, не чаще раза в 2 минуты на канал; наказаний не выносит).
...
2. В разделе Bot включи **Message Content Intent** и **Server Members Intent** (Privileged Gateway Intents) — без первого бот не увидит текст сообщений, без второго не узнает о новичках (а если интент запрошен, но не включён в портале, Discord вообще рвёт соединение с кодом 4014 — тогда либо включи его, либо поставь `Discord__GuildMembersIntent=false`).
...
- `OpenAI__ImageModel` — модель генерации картинок (по умолчанию `gpt-image-1`).
- `SystemPromptFile` — путь к файлу с системным промптом личности; перечитывается при каждом ответе, так что правки применяются без перезапуска. Этот же промпт (вместе с правилами сервера и заметками об участниках в канале) получает голосовая Мурка при `!войс`.
- `StateFile` — путь к файлу состояния (история каналов + «активные» каналы); на VPS стоит указать вне `out/`, например `/data/discordchatbot-state.json`.
- `Discord__GuildMembersIntent` — `false` отключает привилегированный Server Members Intent (и приветствие новичков вместе с ним).
